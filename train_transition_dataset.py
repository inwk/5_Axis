...
def _evaluate_octree_metrics(model: GraphSdfPlanningModel, batch: dict, device: torch.device) -> dict[str, float]:
    ...
    out = model.forward_octree(
        ...
        tool_kind_id=batch.get("tool_kind_id").to(device) if batch.get("tool_kind_id") is not None else None,
        ...
        tool_diameter_norm=batch.get("tool_diameter_norm").to(device) if batch.get("tool_diameter_norm") is not None else None,
        tool_radius_norm=batch.get("tool_radius_norm").to(device) if batch.get("tool_radius_norm") is not None else None,
        tool_length_norm=batch.get("tool_length_norm").to(device) if batch.get("tool_length_norm") is not None else None,
